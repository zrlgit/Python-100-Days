...
![](../Day66-80/res/baidu\_stock\_ma5.png)
...
![](../Day66-80/res/baidu\_double\_MA.png)
...
$$
ho = \frac {\sum_{i=1}^{n}(X_i - \bar{X})(Y_i - \bar{Y})} {\sqrt{\sum_{i=1}^{n}(X_i - \bar{X})^2} \sqrt{\sum_{i=1}^{n}(Y_i - \bar{Y})^2}}
$$
...
其中，$d\_{i}=\operatorname {R} (X\_{i})-\operatorname {R} (Y\_{i})$，即每组观测中两个变量的等级差值，$n$为观测样本数。
...
![](../Users/Hao/Desktop/Python-Data-Analysis/res/boston\_house\_price.png)

> **说明**：上面代码中使用了相对路径来访问 CSV 文件，也就是说 CSV 文件在当前工作路径下名为`data`的文件夹中。如果需要上面例子中的 CSV 文件，可以通过下面的百度云盘地址进行获取。链接：[https://pan.baidu.com/s/1rQujl5RQn9R7PadB2Z5g\_g?pwd=e7b4](https://pan.baidu.com/s/1rQujl5RQn9R7PadB2Z5g\_g?pwd=e7b4)，提取码：e7b4。

可以看出，该数据集中包含了诸多影响房价的特征，包括犯罪率、一氧化氮浓度、平均房间数、低收入人群占比等，其中`PRICE`代表房价，具体情况如下所示。

![](../Day66-80/res/boston\_house\_price\_features.png)
...
![](../Day66-80/res/boston\_person\_correlation.png)
...
![](../Day66-80/res/boston\_spearman\_correlation.png)
